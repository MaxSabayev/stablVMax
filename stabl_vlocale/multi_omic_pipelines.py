...
def cv_on_existing_feats(
    ...
    for m in models:
        dfm = pd.DataFrame({
            ...
            "Fold nb of features": [len(f) for f in feats_dict[m]]
    ...
    for train, test in (tbar := tqdm(
        ...
        for model in filter(lambda x: "STABL" in x, models):
            ...
            if len(fold_selected_features[model]) > 0:
                ...
                if task_type == "binary":
                    #predictions = clone(logit).fit(X_train, y_train).predict_proba(X_test)[:, 1].flatten()
                    predictions = clone(XGBClassifier(use_label_encoder=False, eval_metric="logloss", n_estimators=600, random_state=42)).fit(X_train, y_train).predict_proba(X_test)[:, 1].flatten()
...
# multi_omic_stabl_cv_noe_test: version avec "skip selection" + comparaison cv_on_existing_feats
        ...
        for model_name in filter(lambda x: "STABL" in x, models):
            ...
            if len(feats) > 0:
                ...
                if task_type == "binary":
                    ...
                    predictions = clone(XGBClassifier(use_label_encoder=False, eval_metric="logloss",
